...
export default defineEventHandler(async (event) => {
  ...
  if (payload.app_role !== 'admin' && payload.app_role !== 'client_admin') {
    throw createError({ statusCode: 403, message: 'Only admins can delete staff members.' })
  }
  ...
  try {
    const existing = await pgrestAdmin<any[]>('/staff_members', {
      query: { id: `eq.${id}`, select: 'id,name,surname,owner_user_id,owner_company_id' }
    })
    ...
    throw createError({ statusCode: 500, message: 'Failed to delete staff member.' })
  }
})
